...
RGB getColorForChar(char c, int lowestASCII, int range) {
    ...
    // Generate RGB values based on the normalized value
    unsigned char r = static_cast<unsigned char>(normalizedValue * 255);
    unsigned char g = static_cast<unsigned char>((1 - normalizedValue) * 255);
    unsigned char b = static_cast<unsigned char>(std::abs(normalizedValue - 0.5) * 2 * 255);
...
int main() {
    ...
    for (char c : intention) {
        RGB color = getColorForChar(c, lowestASCII, range);
